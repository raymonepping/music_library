    ...
    (async () => {
      try {
        ...
        const countRs = await client.execute(
          `SELECT COUNT(*) AS n FROM ${ks}.artists`,
        );
        ...
        outer: do {
          ...
          for (const r of rs.rows) {
            ...
            if (cfg.type === "int") {
              ...
              await client.execute(
                insert,
                [b, colVal, nameLc, r.artist_id, name, r.images || null],
                { prepare: true },
              );
            } else {
              if (cfg.column === "name_lc") {
                ...
                await client.execute(
                  insert,
                  [v, r.artist_id, name, r.images || null],
                  { prepare: true },
                );
              } else {
                ...
                await client.execute(
                  insert,
                  [v, nameLc, r.artist_id, name, r.images || null],
                  { prepare: true },
                );
              }
            }
        ...
        if (j && j.status !== "canceled" && j.status !== "error") {
          ...
          logger.info(
            `[iterate] build done by=${by}, inserted=${inserted}, table=${table}`,
          );
        }
      } catch (err) {
...
function getStatus(req, res) {
  ...
  const percent = job.total
    ? Math.min(
        100,
        Math.round((Number(job.inserted) / Number(job.total)) * 100),
      )
    : job.status === "done"
      ? 100
      : 0;
...
async function getTop(req, res) {
  ...
  const by = String(req.query.by || "popularity")
    .trim()
    .toLowerCase();
  const cfg = SORTS[by];
  if (!cfg)
  ...
  try {
    ...
    if (msg.includes("does not exist")) {
      return res
        .status(404)
        .json({ error: "Derived table not found; run /iterate/build first" });
    }
    return res.status(500).json({ error: "internal error" });
  }
}
